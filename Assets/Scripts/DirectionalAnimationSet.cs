...
[CreateAssetMenu(fileName = "DirectionalAnimationSet", menuName = "player_0/DirectionalAnimationSet")]
public class DirectionalAnimationSet : ScriptableObject
{
    ...
    public AnimationClip GetFacingClip(Vector2 facingDirection){
        Vector2 closestDirection = GetClosesDirection(facingDirection);
    ...
    public Vector2 GetClosesDirection (Vector2 inputDirection){
        Vector2 normalizedDirection = inputDirection.normalized;
        Vector2 closestDirection = Vector2.zero;
        float closestDirectance = 0f;
        bool firstSet = false;
        ...
        for(int i =0;i<directionsToCheck.Length; i++){
            if(!firstSet){
                ...
                closestDirectance = Vector2.Distance(inputDirection, directionsToCheck[i]);
                firstSet = true;
            }
            else
            {
                ...
                if(nextDistance < closestDirectance){
                    closestDirectance = nextDistance;
                    closestDirection = directionsToCheck[i];
                }
            }
        }
